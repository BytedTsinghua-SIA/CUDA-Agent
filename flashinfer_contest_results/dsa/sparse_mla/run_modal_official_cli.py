"""用官方 flashinfer-bench CLI 在 Modal B200 上跑 DSA sparse-MLA。

功能：
  构造只包含 DSA sparse attention 的最小 TraceSet，加入指定 solution，
  然后执行官方形式的 `flashinfer-bench run`。

参数：
  --probe：只打印远端 Python/CUDA/FlashInfer-Bench 环境，不跑 benchmark。
  --solution：选择 ours 或 ako；默认 ours。
  --max-workloads：跑前 N 个 workload；默认 0，表示跑全部 23 个。
  --warmup-runs/--iterations/--num-trials：传给 `flashinfer-bench run` 的计时参数。
...
AKO_KERNEL_PATH = SCRIPT_DIR / "kernel_ako4x_hybrid_pdl_v2.py"

REMOTE_ROOT = Path("/root/dsa_sparse_official_cli")
REMOTE_INPUT_DATA = REMOTE_ROOT / "input_contest_data"
REMOTE_KERNEL = REMOTE_ROOT / "kernel.py"
REMOTE_AKO_KERNEL = REMOTE_ROOT / "kernel_ako4x_hybrid_pdl_v2.py"
REMOTE_RUN_DATA = Path("/tmp/dsa_sparse_official_cli_dataset")
...
SOLUTION_CONFIGS = {
    "ours": {
        "name": "cuda_agent_dsa_sparse_mla",
        "author": "cuda_agent",
        "entry_point": "kernel.py::run",
        "language": "triton",
        "source_path": "kernel.py",
        "remote_kernel": REMOTE_KERNEL,
        "description": "CUDA-Agent DSA sparse-MLA Triton kernel.",
    },
    "ako": {
        "name": "ako4x_dsa_sparse_hybrid_pdl_v2",
        "author": "ako4x",
        "entry_point": "kernel_ako4x_hybrid_pdl_v2.py::run",
        "language": "python",
        "source_path": "kernel_ako4x_hybrid_pdl_v2.py",
        "remote_kernel": REMOTE_AKO_KERNEL,
        "description": "AKO4X hybrid_pdl_v2 DSA sparse attention kernel.",
    },
}
...
app = modal.App("cuda-agent-dsa-sparse-official-cli")

image = (
    modal.Image.from_registry(MODAL_IMAGE_REGISTRY, add_python=MODAL_PYTHON)
    .env({"CUDA_HOME": "/usr/local/cuda"})
    # Modal 的 Python 层看不到裸镜像里的 FIB 包；这里沿用 AKO4X 的官方兼容镜像层。
        ...
        "sources": [
            {
                ...
                "content": config["remote_kernel"].read_text(),
            }
        ],
    }
...
print("safetensors", maybe_version("safetensors"))
...
for mod in ("flashinfer_bench", "flashinfer", "deep_gemm"):
    try:
        m = __import__(mod)
        print("import", mod, "ok", getattr(m, "__file__", ""))
    except Exception as exc:
        print("import", mod, "failed", type(exc).__name__, str(exc))
"""
    proc = subprocess.run(
        ["python3", "-c", code],
        text=True,
    ...
    cli_path = shutil.which("flashinfer-bench")
    if cli_path is None:
