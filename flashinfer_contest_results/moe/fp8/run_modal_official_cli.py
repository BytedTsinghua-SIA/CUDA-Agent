...
功能：
  构造只包含 MoE FP8 的最小 TraceSet，加入当前 kernel.py
  作为 VibeCUDA solution，然后执行官方形式的 `flashinfer-bench run`。
...
SOLUTION_NAME = "vibecuda_moe_fp8"
AUTHOR = "vibecuda"
...
def _write_cuda_agent_solution(dataset_dir: Path) -> Path:
    """把当前 kernel.py 打包成 flashinfer-bench Solution JSON。"""
    solution = {
        "name": SOLUTION_NAME,
        ...
        "description": "CUDA-Agent MoE FP8 Triton kernel.",
        "sources": [{"path": "kernel.py", "content": REMOTE_KERNEL.read_text()}],
    }
    out_dir = dataset_dir / "solutions" / AUTHOR / OP_TYPE / DEFINITION
...
def _prepare_dataset(max_workloads: int | None) -> Path:
    ...
    _write_cuda_agent_solution(REMOTE_RUN_DATA)
...
def _summarize_results(dataset_dir: Path) -> str:
    ...
    if common:
        ...
        for uuid in common:
            ...
            lines.append(
                ...
                f"baseline_source={baseline_sources.get(uuid, 'unknown')} "
...
def run_official_cli(
    ...
    dataset_dir = _prepare_dataset(None if max_workloads <= 0 else max_workloads)
    ...
    summary = _summarize_results(dataset_dir)
